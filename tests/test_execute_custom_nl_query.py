...
def test_execute_custom_nl_query(client, mock_openai_client, db_session):
    query_request = QueryRequest(
        natural_language_query="What's the current name of person id: d59abfc4-3aae-4e29-875b-7b56e021ad42?"
    )
    # Insert test data as the setup
    db_session.execute(
        "INSERT INTO people (id, name) VALUES ('d59abfc4-3aae-4e29-875b-7b56e021ad42', 'Test User')"
    )
    db_session.commit()
...
def test_sql_execution_error(client, mock_openai_client):
    query_request = QueryRequest(
        natural_language_query="What's the previous name of person id: d59abfc4-3aae-4e29-875b-7b56e021ad42?"
    )

    # Mocking a SQLAlchemy execution failure
    with patch(
        "sqlalchemy.orm.session.Session.execute",
        side_effect=SQLAlchemyError("SQL execution error"),
    ):
        response = client.post(
            "/execute_custom_nl_query", json=query_request.model_dump()
        )
