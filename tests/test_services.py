...
def test_rename_person(db_session: Session):
    ...
    person_data = PersonRenamed(
        person_id=uuid.UUID(person_id), name="Updated Name", timestamp=datetime.now()
    )
...
def test_remove_person(db_session: Session):
    ...
    person_data = PersonRemoved(
        person_id=uuid.UUID(person_id), timestamp=datetime.now()
    )
...
def test_translate_nl_to_sql(mock_openai_client, mock_openai_response):
    nl_query = (
        "What's the current name of person id: d59abfc4-3aae-4e29-875b-7b56e021ad42?"
    )
